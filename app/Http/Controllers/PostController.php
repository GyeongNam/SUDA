...
class PostController extends Controller
{
  ...
  public function board_list(Request $request){
    ...
    if($mypost=="내가쓴글"){
      $data = DB::table('post')->where('writer',$request->userid)->where('post_activation',1)->orderBydesc()->get();
      return json_encode($data,JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);;
    }
    $data = DB::table('post')->where('categorie_num',$categorie)->where('post_activation',1)->orderBydesc()->get();
    return json_encode($data,JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);
